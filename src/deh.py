...
class DEH():
    ...
    def set_origin_pix(self, data):
        for n in self.nodes:
            self.nodes[n].origin_pix=[]
            if len(n) == self.get_depth():
                ldata = self.get_ldata(data)
                idnum = np.argmin(np.sum((self.nodes[n].classifier - ldata)**2, axis=1))
                self.nodes[n].origin_pix.append(idnum)
    ...
    def svm_desparsify(self, data, split_var=()):
        if len(split_var) == 0:
            split_var = data
        self.set_origin_pix(data)
    ...
    def initialize_splitters(self, dat, level = -1):
        if level < 0:
        ...
        for n in init_nodes:
            if n+'1' in self.nodes:
                ...
                test1.fit(X,y)
                self.nodes[n].splitter = test1.coef_[0], test1.intercept_[0]
    ...
    def set_neighbor_weights(self, data, saturation=()):  
        ldata = self.get_ldata(data)
        base_norm = np.sum(ldata**2, axis=1)
        rel_data = self.neighbors > -1
        if len(saturation)>0:
            rel_data = rel_data & ~saturation
        expected_errs = np.zeros(data.shape[0])
        self.full_weights = np.zeros(data.shape[0]) + 1
        expected_errs[rel_data] = np.sum((ldata[rel_data] - ldata[self.neighbors[rel_data]])**2, axis=-1)/base_norm[rel_data]
    ...
    def simple_predict(self, image, all_nodes = False, weights = (), top_map = (), level=-1):
        #if image.shape[0] != self.weights.shape[0]:
        if len(weights) == 0:
            try:
                self.weights = self.wf(image)
        ...
        if all_nodes:
            ...
            nodes = [n for n in self.nodes if len(n)==level]
        else:
    ...
    def update_spectra_old(self, data, level, utype='spatial'):
        S = []
        rel_nodes = []
        if utype=='spatial':
            for i in self.nodes:
    ...
    def update_from_level_S_V(self, data, beta = 0.1, alg='complex', a_len_max=0.01, n_update_points=0,
                             attenuation = 1, levels=(-1,), occs=(), split_var=(), node_list =()):
        ...
        for level in levels:
            ...
            if len(node_list)==0:
                ...
                incl_nodes = [n for n in node_list if len(n) == level]
            #eL = self.remainder_at_level(data, level)
            #for pix in data:S
            #if '1' in incl_nodes:
            #    incl_nodes = ['1']
            for n in incl_nodes:
                ...
                if n_update_points > 0:
                    ...
                    self.subsamp = indices[om][update_pix]
                    ldata = data[om][update_pix]
                    # if there aren't enough datapoints to update the spectra, include points from one class above
                    if len(indices[om][update_pix])<len(self.nodes):
                        om = occs[n[:-1]]
                        om = om > 0
                        update_pix = np.random.choice(om.sum(), np.minimum(n_update_points, np.sum(om)))
                        self.subsamp = indices[om][update_pix]
    ...
    def equiliberate_node_and_children(self, data, node, n_update_points=0, n_runs=10, split_var=(),
        ...
        for i in range(n_runs):
            ...
            if (spatial and spectral):    
                ...
                lmda = classify_from_partitionII(lmda_O)
                lmda_full = np.zeros(data.shape[0])
                lmda_full[rel_data] = lmda
                prob_map[node+'1'][:] = prob_map[node]*lmda_full
                lmda_full[rel_data] = 1-lmda
                prob_map[node+'0'][:] = prob_map[node]*lmda_full


    
    def one_node_grad_update(self, data, node, n_update_points=0):
        if n_update_points > 0:
            update_pix = np.random.choice(len(data), n_update_points)
            self.node_grad(node, data[update_pix], var='W')
            self.node_grad(node, data[update_pix], var='h')
        else:
            self.node_grad(node, data, var='W')
            self.node_grad(node, data, var='h')
    ...
    def quick_alt(self, data, beta=0.1, tol=0.01, n_update_points=200, sampling_points=(),
        ...
        while (delta > tol)&(i<max_iter):
            i+=1
            if self.cyclic_order>-1:
                #not finished implementing
                split_var_c = split_var[i//self.cyclic_order]
    ...
    def node_grad(self, node, data, scaling_factor=1, var='W', metropolis=False,
        ...
        if self.use_bsp:
            if self.both_sides_pure(node):
                r = self.reg/np.minimum(self.level_limit, self.get_depth())
            else:
                r = 0
        else:
            r = self.reg/np.minimum(self.level_limit, self.get_depth())
        ...
        if var=='W':
            ...
            for i in range(start_layer,depth+1):
                ...
                if metropolis:
                    ...
                    werr = factors[i]*np.sum(np.multiply(eL, out), axis=1)
        ...
        for i in range(start_layer, depth + 1):#range(len(node)+1,depth+1):
            ...
            werr = factors[i]*np.multiply(self.weights, werr)
            
            werr += rloc*Areg#*self.nodes[node].map #+
            if self.use_bonus_boost:
                bonus_boosts = np.array([self.nodes[n].bonus_boost for n in nodes_level_i])
            ...
            d_sum = np.multiply(factors[i]*np.sum(out**2, axis=1),self.weights) - rloc*np.sum(out_reg**2, axis=1) # - r*np.sum(out_reg**2, axis=1)
            ...
            if metropolis:
                num += werr_w#*self.nodes[node].map
                denom += nl*self.nodes[node].map#actors[i]*
            else:    
                num += werr_w*self.nodes[node].map#factors[i]*
                denom += nl*self.nodes[node].map**2#factors[i]*
        ...
        if False:
            ...
            for i, j in enumerate(splits):
                ...
                for k in range(start_layer, depth + 1):
                    ...
                    #    plt.show()
                    e_type1 = np.sum(np.multiply((eL**2).T, self.weights), axis=0).astype(np.float64).mean()
                    e_type2 = -rloc * np.sum(np.multiply(A.T**2, self.weights), axis=0).astype(np.float64).mean()
    ...
    def one_step_cyclic(self, data, scaling_factor=1, n_update_points=0, lowest=False, prob_map=(),
                       split_var=(), both=True, only_ends=False, levels=(), node=()):
        #node should be input as a single 
        if len(split_var)==0:
            split_var = data
                        ...
                        if n_update_points > 0:
                            ...
                            if both:
                                self.node_grad(n, data[pm][update_pix], scaling_factor=scaling_factor, var='W',
                                              split_var=split_var[pm][update_pix],
                                              only_ends=only_ends)
    ...
    def quick_reset(self, node):
        lo = self.nodes[node].lmda_O
        los = np.sort(lo)
        rescale = 2*np.maximum(lo.max()-0.5, -(lo.min()-0.5))
        rescale = np.maximum(rescale, 1)
        self.nodes[node].splitter = [self.nodes[node].splitter[0]/rescale,
                                     self.nodes[node].splitter[1]/rescale]
    ...
    def sparse_grow_node(self, data, split_var, to_grow):
        ...
        if np.sum(self.nodes[to_grow].map==1)>1:
            ...
            pts = self.nodes[to_grow].map>0.5
        else:
            pts = self.nodes[to_grow].map>0
        a, e0, e1 = quick_split(data[pts].astype(np.float64), even=False)
        splitter_a = classifiers_2_svm(e0, e1)
        out = (data[pts]@splitter_a[0]+splitter_a[1]+1)/2
        #ensure that nothing vanishes
        out_upper = out >= 1
        if out_upper.sum()==0:
            out_upper[np.argmax(out)] = True
        out_lower = out <=0
        if out_lower.sum()==0:
            out_lower[np.argmin(out)] = True
        mean0 = np.mean(split_var[pts][out_lower], axis=0)
        mean1 = np.mean(split_var[pts][out_upper], axis=0)
        splitter = classifiers_2_svm(mean0, mean1)
        self.nodes[to_grow].splitter = splitter#0*self.nodes[''].classifier, 0
        
        
    def equiliberate(self, image, n_runs=10, n_pts=1000, sampling_points=(), obj_record=[], epsilon = 0.0,
                    split_var=(), increment=False):
        if len(split_var)==0:
            split_var=image
        i = 0
        ...
        for i in range(n_runs):
            ...
            self.switch_training(image, beta=0, tol=1e-12, n_update_points=n_pts, 
                                     ...
                                     A_protection=False, only_ends=False, split_var=split_var)
            if increment:
    ...
    def gentle_sparsify(self, data, n_points, mpp_tol, n_runs, sampling_points=(), split_var=()):
        ...
        for level in range(depth):
            self.level_limit=level+1
            self.sparsify(data, sampling_points=sampling_points, obj_record=[obj],
                          n_points=n_points, mpp_tol=mpp_tol, step_delta=e_reg_max/n_runs,
                          reg_max=e_reg_max, split_var=split_var, level=level)
        self.level_limit = 255
    ...
    def sparsify(self, data, sampling_points, obj_record, n_points, mpp_tol=0.05, step_delta = 0.01,
             reg_max=1, split_var=(), linear=False, level=-1, max_iter=20, multilevel=False):
        self.reg = 0
        self.use_bsp = True
        scaling_factor = 1
        if len(split_var)==0:
        ...
        #                    split_var=split_var, level=-1)
        deh_mpp = self.mpp(split_var, level=(level+1))
        init_mpp = deh_mpp
        go_on = True
        ...
        while go_on:
            ...
            if multilevel:
                self.sparsity_sweep(data, sampling_points, obj_record, n_points, reg_max/nsteps,
                                reg_max, split_var=split_var, level=level, multilevel=True)
            else:
                self.sparsity_sweep(data, sampling_points, obj_record, n_points, reg_max/nsteps,
                                reg_max, split_var=split_var, level=-1, multilevel=False)
            ...
            if self.prepartition==True:
                ...
                deh_mpp = self.full_mpp(split_var, level=(level+1))
            ...
            if linear:
                ...
                if init_mpp > deh_mpp*1.5:
                    reg_max *= 1.1
                elif init_mpp > deh_mpp*1.3:
                    reg_max *= 1.3
                elif init_mpp > deh_mpp*1.1:
                    reg_max *= 1.5
                else:
                    reg_max *= 2
            ...
            if go_on:
                ...
                scale = self.max_node_scale(split_var)
                scale *= scaling_factor_base
                self.rescale_all_nodes(scale, split_var, less_than=True)
                for n in self.splitting_nodes():
                    if not self.both_sides_pure(n):
    ...
    def mpp(self, image, level=-1):
        if self.mpp_type=='full':
            return self.full_mpp(image, level=level)
    ...
    def gL2_sparsity_sweep(self, data, sampling_points, obj_record, n_update_points, step_delta, reg_max,
                           split_var):
        if len(split_var)==0:
            split_var = data
            
        while self.nu < reg_max:
    ...
    def sparsity_sweep(self, data, sampling_points, obj_record, n_update_points, step_delta=0.01, reg_max = 1,
                      split_var=(), level=-1, multilevel=False):
        deh_mpp = 0.1
        if len(split_var)==0:
            split_var = data
        ...
        while self.reg < reg_max:
            ...
            if level==-1:
                ...
                self.multi_level_updates(data, 1, n_pts=n_update_points, level=level,
                                          update_spectra=True, split_var=split_var)
            else:
                self.single_level_updates(data, 1, n_pts=n_update_points, level=level,
                                          update_spectra=True, split_var=split_var)
            
            deh_mpp = self.mpp(split_var)
    ...
    def exhaustive_grow_network(self, indata, n_update_pts= (0,), mpp_tol=0.05, sampling_points=(),
        ...
        d_norm = self.get_ldata(data)#(data.T/np.sqrt(np.sum(data**2, axis=-1))).T
        self.sparse_grow_node(d_norm, split_var, '')
        #self.simple_predict(data)
        #self.binarize_lmdas()
        #self.lmda_2_map()
        #self.display_level(self.get_depth())
            ...
            for en in endmembers:
                ...
                try:
                    ...
                    self.nodes[en].deh.init_node_split(en,
                                                        ...
                                                        n_pts=n_update_pts[0],
                                                      split_var=split_var)
                    #self.nodes[en].deh.init_node_split(en[:-1],
                    ##                                    data,
                    # #                                   n_runs,
                    #                                    n_pts=n_update_pts[0])
                ...
                try:
                    ...
                    #e_reg_max = obj_record[-1][0] / (np.sum(S**2, axis=0).mean()-1/len(S))
                    #e_reg_max = self.est_reg_max_full(data, mpp_tol)#, self.partitionmember)
                    #self.nodes[en].deh.sparsify(data, sampling_points=sampling_points, obj_record=obj_record,
                    #         n_points=n_update_pts[0], reg_max=e_reg_max, mpp_tol=mpp_tol, step_delta=e_reg_max/n_runs,
                    #             split_var=split_var)
                    self.nodes[en].deh.gentle_sparsify(data, n_update_pts[0], mpp_tol, n_runs, split_var=split_var)
                    ...
                    #for level in range(self.get_depth()-1, -1, -1):
                    #    self.nodes[en].deh.de_sparsify_one_level(data, n_runs=n_runs,
                    #                                             n_points=n_update_pts[0],
                    #                                             mpp_tol=mpp_tol,
                    #                                            level=level)
                    #    self.reg = 0
                    #    self.nodes[en].deh.equiliberate(data, 
                    ...
                    #                                epsilon=0)
                    for n in self.nodes:
                        self.nodes[en].deh.nodes[n].classifier = self.nodes[n].classifier
                    self.nodes[en].deh.svm_gentle_desparsify(data, n_runs, n_update_pts[0], split_var=split_var)
                    # for everything but the new classes, go back to the main estimate
                    ...
                    #S=self.nodes[en].deh.simple_predict(data)
                    #e_reg_max = obj_record[-1][0] / (np.sum(S**2, axis=0).mean()-1/len(S))
                    #e_reg_max = self.est_reg_max_full(data, mpp_tol)#, self.partitionmember)
                    #halftol = mpp_tol + (1-mpp_tol)/2
                    #mpp = self.nodes[en].deh.full_mpp(data)
                    #self.nodes[en].deh.sparsify(data, sampling_points=sampling_points, obj_record=obj_record,
                    #         n_points=n_update_pts[0], reg_max=e_reg_max, mpp_tol=2/3*mpp, step_delta=e_reg_max/n_runs)
                    ...
                    #mpp = self.nodes[en].deh.full_mpp(data)
                    #self.nodes[en].deh.sparsify(data, sampling_points=sampling_points, obj_record=obj_record,
                    #         n_points=n_update_pts[0], reg_max=e_reg_max, mpp_tol=1-2*(1-mpp), step_delta=e_reg_max/n_runs)
        ...
        S=self.simple_predict(split_var)
        self.display_level(self.get_depth())
        #e_reg_max = obj_record[-1][0] / (np.sum(S**2, axis=0).mean()-1/len(S))#self.est_reg_max_full(data, mpp_tol)#, 
        #e_reg_max = self.est_reg_max_full(data, mpp_tol)#, self.partitionmember)
        #self.sparsify(data, sampling_points=sampling_points, obj_record=obj_record,
        #         n_points=n_update_pts[0], reg_max=e_reg_max, mpp_tol=mpp_tol, step_delta=e_reg_max/n_runs)
        self.gentle_sparsify(data, n_update_pts[0], mpp_tol, n_runs, split_var=split_var)
    ...
    def gentle_exhaustive_grow_network(self, indata, min_split=1, n_update_pts= (0,), sampling_points=(),
        ...
        self.start=time.time()
        self.n_update_pts = n_update_pts[0]
        self.training='grow_network_gentle'
    ...
    def restart_gentle_exhaustive_growth(self, indata, n_update_pts= (0,), sampling_points=(),
                          n_runs=20, save=False, save_name='', saturation=(), split_var=(),
                               scale_spectra=False):
    ...
    def gentle_exhaustive_grow_network_gL2(self, indata, min_split=1, n_update_pts= (0,), sampling_points=(),
                          n_runs=20, save=False, save_name='', saturation=(), split_var=(),
                               scale_spectra=False):
    ...
    def init_node_split(self, node, data, n_runs, n_pts, split_var=()):
        insp_node = node
        L = len(node)
        if len(split_var)==0:
            split_var=data
        data_points = (self.nodes[node].map == 1)&(self.weights>0)
        if data_points.sum() > len(self.nodes):
            print("starting split with points ", data_points.sum())

            node_trainer = self.copy()
            node_trainer.aa=False
            ...
            for n in self.nodes:
                if n[:L] == node:
                    ...
                    try:
                        node_trainer.nodes[n[L:]] = Node(spatial_map = np.ones_like(data[data_points]),
                                                    classifier = self.nodes[n].classifier)
                        node_trainer.nodes[n[L:]].splitter = (self.nodes[n].splitter[0],
                                                              self.nodes[n].splitter[1])
                    except AttributeError:
                        node_trainer.nodes[n[L:]] = Node(spatial_map = np.ones_like(data[data_points]),
                                                    classifier = self.nodes[n].classifier)
            ...
            while go_on:
                ...
                S = node_trainer.simple_predict(split_var[data_points])
                max_per_endm = S.max(axis=1)
                if max_per_endm.min()==1:
                    go_on=False
    ...
    def exhaustive_full_run(self, rdata, n_pts=(1000,0), mpp_tol=0.3, n_runs=10, saturation=(), split_var=()):
        self.sample_sizes = n_pts
        self.neighbors = quick_nn(rdata.reshape(self.plot_size + (-1,)), k_size=1).flatten()
        self.set_neighbor_weights(rdata, saturation)
        if len(saturation)==len(rdata):
            self.full_weights[saturation] = 0
            data = rdata[~saturation]
            self.full_weights=self.full_weights[~saturation]
        ...
        self.equiliberate(data, obj_record=obj_rec, n_runs=n_runs, n_pts=n_pts[0], epsilon=0, split_var=split_var)
        
        self.simple_predict(split_var)
        self.display_level(self.get_depth())
        self.accepted_network_stablization(data, n_runs=n_runs, n_pts=n_pts, obj_record=obj_rec, sampling_points=(),
                                 mpp_tol=mpp_tol, name=self.save_name, split_var = split_var)
    ...
    def merge_splitters(self, proportion, old_nodes):
        p = proportion
        for n in self.nodes:
            if n+'1' in self.nodes:
                oldsp = old_nodes[n].splitter
                self.nodes[n].splitter = ((1-p)*self.nodes[n].splitter[0] + p*oldsp[0],
                                          (1-p)*self.nodes[n].splitter[1] + p*oldsp[1])
    
    
    def accepted_network_stablization(self, data, n_runs=100, n_pts=(0,), obj_record=(), sampling_points=(),
                                 mpp_tol=0.05, name='default', split_var=()):
        if len(split_var)==0:
            split_var = data
        if len(obj_record)==0:
            obj_record = [0]
    ...
    def old(self):
        ...
        self.update_spectra=False
        self.equiliberate(data, obj_record=obj_record, n_runs=n_runs,epsilon =0,
                n_pts=n_pts[0], sampling_points=sampling_points, split_var=split_var)
        self.update_spectra=True
        ...
        for i in range(n_runs):
            self.equiliberate(data, obj_record=obj_record, n_runs=n_runs,epsilon =0,
                        n_pts=n_pts[-1], sampling_points=sampling_points, split_var=split_var)
        if self.save_output==True:
            self.save(name+'_' + 'ppaaa' + '_FINAL' +'.h5')
        ...
        for i in range(n_runs):
            ...
            #e_reg_max = self.est_reg_max_full(data, mpp_tol)#, self.partitionmember)
            #halftol = mpp_tol + (1-mpp_tol)/2
            #mpp = self.full_mpp(data)
            #self.sparsify(data, sampling_points=sampling_points, obj_record=obj_record,
            #         n_points=n_pts[0], reg_max=e_reg_max, mpp_tol=mpp*(i/(1+i)),
            #              step_delta=e_reg_max/n_runs)#self.reg /= 2
            self.equiliberate(data, obj_record=obj_record, n_runs=n_runs,epsilon =0,
                n_pts=n_pts[0], sampling_points=sampling_points, split_var=split_var)
            self.simple_predict(split_var)
            self.display_level(self.get_depth())
    ...
    def de_sparsify(self, image, n_runs, n_points, mpp_tol, obj_record =[], decay=True, split_var=()):
        '''
        ...
        while go_on:
            ...
            deh_mpp1 = self.full_mpp(split_var)#self.collective_mpp(image, mode='min')
            valid = self.weights > 0
            for n in self.nodes:
                if n+'1' in self.nodes:
                    self.increment_ui(n, self.nodes[n].lmda_O[valid])
                    self.increment_li(n, self.nodes[n].lmda_O[valid])
            deh_mpp2 = self.full_mpp(split_var)#self.collective_mpp(image, mode='min')
            ...
            self.display_level(self.get_depth())
            deh_mpp = self.full_mpp(split_var)#self.collective_mpp(image, mode='min')
            i+=1
            ...
            #else:
            #    go_on = deh_mpp < 1 - n_points/len(image)
            if go_on:
        ...
        if decay:
            ...
            self.sparsity_sweep(image, obj_record=obj_record,
                                ...
                                sampling_points=(), split_var=split_var)
            self.reg = 0
    ...
    def level_mpp(self, level):
        lmpp = 1
        for n in self.nodes:
            if len(n)==level:
                lmpp = np.minimum(lmpp, self.node_mpp(n))
        return lmpp
   
    def de_sparsify_one_level(self, image, n_runs, n_points, mpp_tol, level, obj_record =[], decay=False):
        '''
        Should be run after some reg=0 equiliberation
        '''
        self.reg = 0
        S = self.simple_predict(image)
        deh_mpp = self.level_mpp(level)
        i=0
        self.use_bsp=False
        self.update_spectra=False
        ...
        go_on=True
        lnodes = [n for n in self.nodes if len(n)==level]
        prob_map = S
        while go_on:
            prob_map = {}
            for n in self.nodes:
            ...
            for i in range(n_runs):
                for n in lnodes:
                    ...
                    if nmpp < 1:
                        if n+'1' in self.nodes:
                            self.nodes[n].h_update = 0 
                            self.nodes[n].W_update = np.zeros(image.shape[-1],
                                                  dtype=np.float32)
                            #plt.plot(self.nodes[n].splitter[0])
                            if n_points==0:
                                n_points = len(image)
                            ...
                            self.subsamp = update_pix
                            self.node_grad(n, image[pm][update_pix], scaling_factor=1, var='W',
                                              split_var=(),
                                              only_ends=False)
                            self.node_grad(n, image[pm][update_pix], scaling_factor=1, var='h',
                                          metropolis=False, split_var=(),
                                              only_ends=False)
            ...
            S = self.simple_predict(image)
            self.display_level(self.get_depth())
            err = self.remainder_at_level(image, level+1)
            #gain = / 
            
            deh_mpp = self.level_mpp(level)#self.collective_mpp(image, mode='min')
            i+=1
            print("desparsified to ", deh_mpp)
            
            gain = np.sum(np.multiply((err.T)**2, self.weights), axis=0).mean()  / (1-np.sum(S**2, axis=0).mean())#((np.sum(S**2, axis=0).mean())-1/len(S))
            self.reg -= gain
            #print(obj_record[-1][0], np.sum(S**2, axis=0).mean())
    ...
    def shake(self, data, n_runs, n_pts=1000, sampling_points=(), obj_record=(), bound_type='mean',
             split_var=(), bsp=True, increasing=True, penalty = 'gamma', rate=2):
        if len(split_var)==0:
            split_var=data
        if len(obj_record)==0:
            need_to_pop = True
        ...
        self.equiliberate(data, n_runs, n_pts, sampling_points, obj_record, epsilon=0, split_var=split_var)
        if need_to_pop:
            obj_record.pop(0)
        ...
        while go_on:
            ...
            if bound_type=='mean':
                new_bound = new_obj_mean #+ obj_std
            elif bound_type=='max':
                new_bound = new_obj_max
            go_on = new_obj_min < bound
            if i > n_runs**2:
                go_on = False
            #if not self.both_sides_pure(''):
            #    print("impure, equiliberating again")
            #    self.equiliberate(data, n_runs, n_pts, sampling_points, obj_record, epsilon=0, split_var=split_var)
            #    #go_on = False
            print('objs',objs)
    ...
    def increment_lo(self, splitter, split_points):
        '''
    ...
    def single_level_updates(self, data, n_runs, n_pts, level, update_spectra=True, split_var=()):
        '''
        individual steps not stored in the record
        '''
        if len(split_var) == 0:
            split_var = data
        
        # generate probability map of the level 
        self.simple_predict(split_var)
        prob_map = {}
        for n in self.nodes:
            prob_map[n] = self.nodes[n].map

        rel_nodes = [n for n in self.nodes if len(n)==level]
        for i in range(n_runs):
            if update_spectra:
                self.one_step_S(data, alg='simple', n_update_points=n_pts, occs=prob_map, split_var=split_var)
        # update nodes of particular level
            self.one_step_cyclic(data, n_update_points=n_pts, lowest=False, prob_map=prob_map,
                       scaling_factor=self.scaling_factor, node=rel_nodes, split_var=split_var)
            
            
    def multi_level_updates(self, data, n_runs, n_pts, level, update_spectra=True, split_var=()):
        '''
        individual steps not stored in the record
        update of all levels <= specified level
        '''
        if len(split_var) == 0:
            split_var = data
    ...
    def svm_gentle_desparsify(self, data, n_runs, n_pts, split_var=(), shake=False):
        if len(split_var) == 0:
            split_var = data
        #slowly desparsify level-by-level
        #starting with the lowest level
        self.set_origin_pix(data)
        depth = self.get_depth()
        self.initialize_splitters(split_var, level = depth-1)
        
        #desparsify the lowest level forst
        #S = self.simple_predict(split_var)
        #sp = sparsity(S)
        #o_sp = 1
        #j = 0 
        #while j < n_runs:
        self.single_level_updates(data, n_runs, n_pts, level=depth-1,
                                  update_spectra=False, split_var=split_var)
            #S = self.simple_predict(split_var)
            #sp = sparsity(S)
        #    j+=1
        S = self.simple_predict(split_var)
        ...
        # first initialize
        # then update each lower layer
        for L in range(depth-2,-1,-1):
            if shake:
                self.shake(data, n_runs, n_pts=n_pts, bound_type='max')
            self.set_origin_pix(data)
            self.initialize_splitters(split_var, level = L)
            #for level in range(L, depth):
            j=0
            #while j < n_runs:
            self.single_level_updates(data, n_runs, n_pts, level=L,
                                      update_spectra=False, split_var=split_var)
            #j+=1
            S = self.simple_predict(split_var)
            self.display_level(depth)
    ...
    def svm_deresparsify_by_level(self, data, n_runs, n_pts, mpp_tol, split_var=()):
        '''
        Desparsify and resparsify different levels of the deh
        '''
        self.only_ends = True
        ...
        for L in range(depth+1):
            # desparsify nodes of level L based on SVM parameters
               
            self.set_origin_pix(data)
            self.initialize_splitters(split_var, level = L)
            S = self.simple_predict(split_var)
            So = np.array([self.nodes[n].map for n in self.nodes if len(n)==(L+1)])
            # run an equiliberation in which:
            # the splitters of nodes of level L are updated
            # no spectra are updated
            sp = sparsity(S)
            o_sp = 1
            while sp-o_sp < 0:
                self.single_level_updates(data, n_runs, n_pts, level=L, update_spectra=False, split_var=split_var)
                o_sp = sp
                S = self.simple_predict(split_var)
                sp = sparsity(S)
                print("sparsity: ", sp)
            ...
            # run an equiliberation in which:
            # all spectra are updated
            # the splitters of nodes of level L are updated
            o_sp = 1
            while sp-o_sp < 0:
                self.single_level_updates(data, n_runs, n_pts, level=L, update_spectra=True, split_var=split_var)
                o_sp = sp
                S = self.simple_predict(split_var)
                sp = sparsity(S)
                print("sparsity: ", sp)
            ...
            obj = (np.sum((eL.T**2), axis=0)*self.weights).mean()
            e_reg_max = obj / (np.sum(S**2, axis=0).mean()-1/len(S))
            self.sparsify(data, sampling_points=(), obj_record=[[obj]], n_points=n_pts, mpp_tol=mpp_tol, 
                          step_delta = e_reg_max/n_runs, reg_max=e_reg_max, split_var=split_var, linear=False, level=L,
                         multilevel=True)
            #Sf = self.simple_predict(split_var)
            S=self.simple_predict(split_var)
            Sf = np.array([self.nodes[n].map for n in self.nodes if len(n)==(L+1)])
            fig, ax = plt.subplots(1, len(Sf))
            for i in range(len(Sf)):
                ax[i].imshow((Sf-So)[i].reshape(self.plot_size), aspect=self.plot_aspect,
                             vmin=-1, vmax=1, cmap='bwr')
             
    
    
    def svm_independent_desparsification(self, n_runs):                                    
        # make a copy of the network in which to store the updated nodes
        new_node_network = self.copy()
        # save the old original nodes so that they can be re-loaded as needed
        self.old_nodes = self.copy_nodes()
        
        self.only_ends = True
        depth = self.get_depth()
        for L in range(depth+1):
            # load the copy of the original old nodes
            self.nodes = copy.deepcopy(self.old_nodes)
            ...
            # run an equiliberation in which:
            # the splitters of level L are updated
            # no spectra are updated
            self.single_level_updates(data, n_runs, n_pts, level=L, update_spectra=False)
            
            # the new splitters are saved in the network copy
            for n in self.nodes:
                if len(n)==L:
                    new_node_network.nodes[n] = (self.nodes[n].splitter[0],
                                                 self.nodes[n].splitter[1])
            
        # add all the new splitters to the original network
        for L in range(depth):
            for n in self.nodes:
                if len(n)==L:
                    self.nodes[n].splitter = (new_node_network.nodes[n].splitter[0],
                                              new_node_network.nodes[n].splitter[1])
        for L in range(depth):
            # run an equiliberation in which:
            # the splitters of level L are updated
            self.single_level_updates(data, n_runs, n_pts, level=L, update_spectra=False)
    ...
    def grow_network_symmetric(self, indata, n_update_pts= (0,), mpp_tol=0.05, sampling_points=(),
                          n_runs=20, save=False, save_name='default', saturation=(), split_var=(),
                               scale_spectra=False):
        ...
        while keep_growing:
            ...
            #for n in self.nodes:
            #    self.nodes[n].classifier = self.nodes[n].classifier
            #self.svm_gentle_desparsify(data, n_runs, n_update_pts[0], split_var=split_var)
            #self.reg = 0
            #self.equiliberate(data, obj_record=obj_record, n_runs=n_runs, scaling_factor=2, epsilon =0,
            #        n_pts=n_pts[0], sampling_points=sampling_points)
...
def quick_split(data, tol=1e-6, weights = (), ppa=False, even=False):
    ...
    while delta > tol:
        ...
        if ppa:
            j0loc = np.argsort(np.sum((data-j0)**2, axis=1))[0]
            j1loc = np.argsort(np.sum((data-j1)**2, axis=1))[0]
            if j0loc == j1loc:
                j1loc = np.argsort(np.sum((data-j1)**2, axis=1))[1]
            j0 = data[j0loc]
            j1 = data[j1loc]
...
def align_spectra(gt_map, S, gt_e, E):
    '''
